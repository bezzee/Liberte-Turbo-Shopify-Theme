...
var imageFunctions = {
  zoom: function(zoomParent, enableTouch, magnify){
    const $zoomParent = $(zoomParent);
    if ($zoomParent.data('zoom'))
      $('img:not(.zoomImg)', $zoomParent)
        .filter(function () { return !$(this).data('zoomInit'); })
        .data('zoomInit', true)
  ...
  init: function(){
    $('.js-product-slider .products-slider').each(function (index, value) {
      ...
      $(this).flickity({
        ...
        "wrapAround": wrapAround,
        "cellAlign": cellAlign,
        // Abraham: harcoded "false" to hide page dots on the List All Collections page
        "pageDots": false,//usePageDots,
        "contain": true,
        "freeScroll": true,
        "arrowShape": arrowSize,
        "initialIndex": initialIndex
  ...
  createFrame: function(settings){
    ...
    if(settings.provider === 'youtube') {
      ...
      if (settings.posterImage === false && settings.autoPlayVideo === false && is_touch_device() === 0){
        ...
        if (settings.autoPlayVideo && settings.posterImage !== false){
          poster = ' poster="https://' + settings.posterImage;
        }
      }
...
var productPage = {
  init: function(){
    ...
    $('.product_gallery')
      .on('settle.flickity', function () { imageFunctions.zoom('.product_gallery', false); })
      .flickity({
        "wrapAround": "true",
        "cellAlign": "left",
        "draggable": draggable,
    ...
    $('.product_gallery').find('.gallery-cell').each(function() {
      const $this = $(this);
      if (parseInt($this.data('image-height'), 10) < 539) {
        $this.find('.img__container').css('max-width', 'unset');
      } else {
        // Abraham, 9/25/2018: Commented this out because this was stretching images beyond their natural height on mobile
        $this.find('img').css({/*height: '539px',*/ width: 'auto'});
      }
    });


    /*============================================================================
      Filter Images on a product
    ==============================================================================*/

    $.fn.filterByColor = function (color) {
      return this.filter(function () {
        const alt = $(this).find('img').prop('alt').trim();
        return alt.indexOf(color) > -1 || !/\[[a-z ]+]/.test(alt);
      });
    };

    $('.swatch-element.color').prev().change(function() { // data-option-index needs to be color choices swatch

      var color = $(this).next().attr('data-value').toLowerCase();
      color = '[' + color + ']';
      const
        $closest = $(this).closest('.remodal, .main.content'),
        $pg = $closest.find('.product_gallery'),
        flickityOrig = $pg.data('flickityOrig');

      $closest
        .find('.product_gallery_nav .gallery-cell')
        .hide()
        .filterByColor(color)
        .show();
      ...
      if (flickityOrig) {
        $pg
          .flickity('remove', $pg.flickity('getCellElements'))
          .flickity(
            'append',
            flickityOrig
              .removeAttr('style')
        ...
        //if ($pg.length > 1) debugger;
      }

    });

    this.cacheOrigFlickity();
    ...
    {% if settings.product_form_style == "swatches" %}
      function getCheckedSwatches ($form) {
        return $form.find('.swatch_options input:checked');
      }
      function updateProductVariantText ($checkedSwatches) {
        $checkedSwatches
          .closest('.product_section')
          .find('.product-variant-text')
          .text(
            $checkedSwatches
              // Get only the ones that are not for colors
              .next(':not(.color)')
              .prev()
      ...
      $('body').on('change', '.swatch :radio', function() {
        ...
        var $checkedSwatches = getCheckedSwatches(parentForm);
        var option1 = $checkedSwatches.eq(0).val();
        var option2 = $checkedSwatches.eq(1).val() || '';
        var option3 = $checkedSwatches.eq(2).val() || '';
        ...
        // Added by Abraham: update the variant text
        // TODO: Make this work for pages that have multiple products
        updateProductVariantText($checkedSwatches);
    ...
    $('.product_form:eq(0)').each(function () {
        updateProductVariantText(getCheckedSwatches($(this)));
      });
    {% endif %}
  ...
  cacheOrigFlickity: function () {
    const $els = $('.product_gallery');
    $els.each(function () {
      $(this).data('flickityOrig', $($(this).flickity('getCellElements')));
    });
    $els.closest('.remodal, .main.content').find('.swatch-element.color').prev(':checked').change();
  },
...
var newsletter_popup = {
  ...
  open: function(){
    if ({{ settings.newsletter_popup_mobile }} || $(window).width() > 798) {
      ...
      //const $firstIndexSection = $('.index-sections > :eq(0)');
      const $header = $('.header.header-fixed--true');
...
var header = {
  ...
  miniCartCloseTimeout: null,

  // Abraham - added mini cart timer functionality
  closeMiniCart: function() {
    ...
    if ($cartContainer.hasClass('active_link')) {
      ...
      $cartContainer.removeClass('active_link');
    }
  },

  openMiniCart: function(arg) {
    ...
    (is$ ? arg : $('.cart_container:visible')).addClass('active_link');

    if (timeout)
      this.miniCartCloseTimeout = setTimeout(function () { header.closeMiniCart(); }, timeout);
  },

  init: function(){


    // Move the banner on the home page
    const $headerContainer = $('#shopify-section-header').addClass('initialized');
    if ($('body').hasClass('index')) {
      // On the home page, insert the header immediately AFTER the last consecutive video or image overlay section
      $headerContainer.remove();
      const $indexSections = $('.index-sections');
      for (
        var $insertBefore = $('> :eq(0)', $indexSections);
        $insertBefore.is('.video-section, .image-with-text-overlay-section');
        $insertBefore = $insertBefore.next()
      );
      if ($insertBefore.length) $insertBefore.before($headerContainer);
      else $indexSections.append($headerContainer);
    }
    ...
    if ($window.width() > 798) {
      ...
      if ($header.hasClass('header-fixed--true')){
        var $mainNavWrapper = $('.main_nav_wrapper');
        if (!$mainNavWrapper.hasClass('sticky_nav')) {

          /*
          $window.scroll(function () {
            $header.toggleClass('scrolled', !!$window.scrollTop());
          }).scroll();
          */

          // Abraham, 7/16: Headhesive is unnecessary here. We can do it much more performantly manually
          /*$mainNavWrapper
            .clone()
            // Add the same classes that Headhesive previously added
            .addClass('sticky_nav sticky_nav--stick')
            .prependTo('body');

          // Make the original one invisible
          $mainNavWrapper.css('visibility', 'hidden');
          ...
          /*var sticky_nav = new Headhesive('.main_nav_wrapper', {
            ...
            onInit: function() {* /
            // Just run the code that previously was in the "onInit" callback right away
              ...
              {% if settings.menu_position == "inline" %}
    ...
    $(".mini_cart").on("click", function(e) {
      ...
      if($cart_container.hasClass('active_link')) {
        ...
        header.openMiniCart($cart_container);
      }
    ...
    if (true /*$(window).width() <= 798*/) {
      ...
      /**
       * Remove the "active_link" class from whatever element currently has that class,
       * EXCEPT the mini cart
       */
      ...
      $('.dropdown_link').on('mouseover touchstart', function(e) {
        ...
        if(!$(this).hasClass('active_link')) {
          ...
          if($(this).hasClass('mini_cart')) {
            // Abraham, 8/10: show mini cart on click, not hover
...
var searchAutocomplete = {
  init: function(){
    ...
    var searchForms = $('form.search_form, form.search, form.header_search_form').each(function() {
      ...
      $('<div class="search__results-wrapper"><ul class="search__results"></ul></div>').insertAfter($(this));
      ...
      input.attr('autocomplete', 'off').bind('keyup change', function() {
        ...
        var resultsList = form.siblings().find('.search__results');
          ...
          currentAjaxRequest = $.getJSON(searchURL +'*&view=json', function(data) {
            ...
            if(data.results_count == 0) {
              ...
              $.each(data.results, function(index, item) {
                if(index >= {{ settings.search_to_display }}) {
                  return false;
                }
...
$(function() {
  ...
  $('body').on('opened', '.js-remodal-lightbox', function () {
    ...
    var $groupImages = $('.lightbox[rel=' + group + ']')
      .filter(function () {
        return !$(this).closest('.remodal').length;
      })
      .parent()
      .clone();
    ...
    //if (!flktyEnabled){
      flktyGallery.remove(flktyGallery.getCellElements());
  ...
  $('body').on('click', '.search-submit', function() {
      //$(this).parent().submit();
      if ( !$('.search_container').hasClass('_show') ) {
        $('.search_container').addClass('_show');
        setTimeout(function(){
         	$('.search_form input[name=q]').focus();
        },300);
      }
      else {
		$('.search_container').removeClass('_show');
      }
  });
  ...
  {% if settings.search_pagination_type == 'load_more' %}
    enableLoadMoreSearch();
  {% endif %}

  {% if settings.search_pagination_type == 'infinite_scroll' %}
    enableInfiniteSearchScroll();
  {% endif %}
      ...
      if (globalQuickShopProduct != 'undefined'){
        ...
        $.each( product.collection_handles, function( value, index ) {
          if (this.toString() == 'new'){
            $('.js-new-banner').html({{ 'collections.general.new' | t | json }});
          }
          if (this.toString() == 'pre-order'){
            $('.js-pre-order-banner').html({{ 'collections.general.pre_order' | t | json }});
          }
          if (this.toString() == 'coming-soon'){
            product.price = {{ 'collections.general.coming_soon' | t | json }};
          }
        ...
        //No variants
          ...
          {% if settings.sale_banner_enabled %}
            if (product.compare_at_price > product.price){
             $('.js-sale-banner').html({{ 'collections.general.sale' | t | json }});
            }
          {% endif %}
          ...
          if (product.available == false){
            $('.js-sold-out').html({{ 'products.product.sold_out' | t | json }});
          }
          ...
          if (product.available == true) {
            ...
            if (product.price == {{ 'collections.general.coming_soon' | t | json }}){
    ...
    populateGallery: function(product){
      ...
      function addCarouselGalleryImages(){
        $.each( product.images, function( i, imgPath ){
          ...
          var img = $('<img src="'+ carouselSizedImg + '" />').prop('alt', product.image_alts[i]);
          var $carouselCellElems = $('<div class="gallery-cell"></div>').append(img);
  ...
  function ajaxUpdateCart(lineID, quantity, parent) {
    ...
    // header.init() clones the entire sidebar cart view for the mobile header, including the template, so we remove both, but save only one
  const
      $cartItemTemplate = $('.cart_item_template').remove().eq(0).removeClass('cart_item_template').show(),
        $lineItemTemplate = $('.line-item', $cartItemTemplate).remove();
  function refreshCart(cart) {
     ...
     * @param {number}  money
     */
    function setFormattedMoney ($el, money) {
      $el.text(Shopify.formatMoney(money, $('body').data('money-format')));
    }
    ...
    const
      ...
      $jsCartItems = $('.js-cart_items');

    // Show the cart count
    $('.cart_count').text(itemCount);

    // Clear all the existing cart items
    $jsCartItems.children().remove();


    $('.js-empty-cart__message').toggleClass('hidden', !!itemCount);
    $('.js-cart_content__form').toggleClass('hidden', !itemCount);
    ...
    if (itemCount) {
      ...
      cart.items.forEach(function (item, i) {
        ...
        const
                  // Note that $item.length == 2 - one in the mobile cart view and one in the desktop view
          $item = $cartItemTemplate
            .clone()
            .removeAttr('hidden')
            .show()
            .appendTo($jsCartItems),
          image = item.image,
          title = item.title;

        // Links
                $('.item_link', $item).prop('href', item.url);
        ...
        $('.cart_image', $item)
          ...
          .prop({
            alt: title,
            src: image.replace(/(\.[^.]*)$/, "_compact$1").replace('http:', '')
          });

        // Item title
        $('.item_title', $item).text(title);

        // Line items
        $.each(item.properties, function (title, value) {
          if (!value) return;
          const $lineItem = $lineItemTemplate.clone().insertBefore($lineItemTemplate);
          $('.line-item_title', $lineItem).text(title);
          $('.line-item_value', $lineItem).text(value);
        });
        ...
        // Price
        setFormattedMoney($('.price .money', $item), item.price);

        // We'll worry about this if it needs to be worried about
    ...
    setFormattedMoney($('.js-cart_subtotal .money'), cart.total_price);
  ...
  function qtyChangeEvent ($el, remove) {
    ajaxUpdateCart($el.data('line-id'), remove ? 0 : $el.val(), $el.parents('#cart_form, .cart_content'));
  }
  $('body')
    .on("change", "input.quantity", function() {
        qtyChangeEvent($(this));
    })
    /*.on("change", ".cart_content input.quantity", function() {
        ajaxUpdateCart($(this).data('line-id'), $(this).val(), $(this).parents('.cart_content'));
    })*/
    .on('click', '.remove_from_cart', function () {
      if (confirm('Are you sure you want to remove this item from your cart?'))
        qtyChangeEvent($(this).closest('.cart_item').find('.quantity'), true);
      return false;
    });
 ...
 * License: MIT
 */
// Abraham, 7/16: We don't need Headhesive because we can do a sticky header much more performantly manually
          ...
          success: function(data) {
            ...
            if(data.meta.code === 200 && data.data.length) {
              ...
              for(var i = 0; i < data.length; i++) {
                ...
                if(item) {
                  item = '<div class="three columns instagram__item">'+item+'</div>';
                }
                if(item !== '') {
                  s.el.append(item);
                  count++;
  ...
  Size calculator
  =====================*/
$('.size-calculator').submit(function() {
  ...
  /**
   * Get the rounded value from one of the size fields in this form. If the fractional part of the value is <= .5, it is rounded
   * down. Otherwise, if the decimal part of the value is > .5, it is rounded up.
   */
  function getRoundedMeasurement (name) {
    const
      size = $('[name="' + name + '-measurement"]', form).val(),
      flooredSize = Math.floor(size);
    return size - flooredSize <= .5 ? flooredSize : Math.ceil(size);
  }

  const
  	form = this,
    bandMeasurement = getRoundedMeasurement('band'),
    bandSize = Math.floor(bandMeasurement / 3) * 2 + 14,
    // A difference of 1 goes to A, and so on...
    cupSize = getRoundedMeasurement('bust') - bandMeasurement + 64;

  $('.bra-result', this)
    .text(
      bandSize >= MIN_BAND_SIZE && bandSize <= MAX_BAND_SIZE &&
      cupSize >= MIN_CUP_SIZE.charCodeAt(0) && cupSize <= MAX_CUP_SIZE.charCodeAt(0)
        ? 'Your bra size is: ' + bandSize + String.fromCharCode(cupSize)
        : 'We couldn\'t determine your bra size.'
    );
  return false;
});
...
/**
 * Added by Abraham: A hacky override to get the money format right :(
 */
{% raw %}
  Shopify._oldFormatMoney = Shopify.formatMoney;
  Shopify.formatMoney = function (value, format) {
    var
      containsAmountNoZeros = format.indexOf('{{amount_no_zeros}}') > -1,
      formatted = Shopify._oldFormatMoney(value, format.replace(/{{amount_no_zeros}}/g, '{{amount}}'));
      return containsAmountNoZeros ? formatted.replace('.00', '') : formatted;
  }
{% endraw %}

// Add waypoints for home page
$('.os-mask-and-zoom').waypoint(function (direction) {
